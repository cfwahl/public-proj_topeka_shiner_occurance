...
df_all <- read_csv("data_fmt/tksn_env_fws_isu.csv") %>% 
  select(SampleID:pH) %>% # select columns from Site_num to pH
...
df_oxbow <- df_all %>% 
  ...
  group_by(site, year) %>%  # grouping by site and year 
  summarize(occurrence = sum(topeka_shiner), # take sum of topeka shiner for each group
            temp = mean(watertemp_c, na.rm = TRUE), # get mean values for these varaibles and remove NAs
  ...
  ungroup() %>% 
  mutate(occurrence = replace(occurrence, occurrence > 0, 1),  # if occurrence is >0 then make 1
         oxbow_id = row_number()) %>% # add unique identifer
  slice(-c(4, 5, 6, 7, 15, 16)) # these sites fall outside connectivity stream network
